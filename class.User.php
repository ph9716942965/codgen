...
class User extends \TAS\Entity
{

    public $UserID, $Username, $Password, $FullName, $FirstName, $LastName, $Email, $UserRoleID;
    ...
    {
        $fields = \TAS\Entity::GetFieldsGeneric($GLOBALS['Tables']['user']);
        ...
        {
            $userid = new \TAS\User($_SESSION['webuserid']);
        }
        else
        {
    ...
    public static function GetFieldsUser($id = 0)
    {
        ...
        $fields['userid']['query'] = "Select concat(firstname,' ',lastname) as name,userid from " . $GLOBALS['Tables']['user'] . " WHERE roleset =3 ORDER BY userid ASC";
    ...
    public static function GetFieldsUserAdmin($id = 0)
    {
        ...
        $fields['userid']['type'] = 'select';
        $fields['userid']['selecttype'] = 'query';
        $fields['userid']['query'] = "Select concat(firstname,' ',lastname) as name,userid from " . $GLOBALS['Tables']['user'] . " WHERE userroleid  !=1 and roleset < 4  ORDER BY userid ASC";
        $fields['userid']['dbID'] = 'userid';
        $fields['userid']['dbLabelField'] = 'name';
        $fields['userid']['required'] = true;
        return $fields;
     ...
     */
    public static function UniqueUser($d, $userid = 0)
    {
      if ($userid == 0) {
    ...
    public function Update($values = array())
    {
        ...
        if (! self::Validate($values, 'user')) {
            ...
            if ($GLOBALS['db']->Update($GLOBALS['Tables']['user'], $values, $values['userid'], 'userid')) {
                return true;
            } else {
                return false;
            }
    ...
    public static function Delete($id)
    {
        ...
        if($roleset=='1')
        {
            $delete = $GLOBALS['db']->Execute("Delete from " . $GLOBALS['Tables']['user'] . " where orgid=" . (int) $orgid . "");
            return true;
        }
        else
        {
    ...
    {
        $rs = $GLOBALS['db']->Execute("Select * from " . $GLOBALS['Tables']['user'] . " where email='" . $email . "' and status=0");
        if (\TAS\DB::Count($rs) > 0) {
            while ($row = $GLOBALS['db']->Fetch($rs)) {
                $newhash = md5(strtolower($row['username']) . strtolower($row['adddate']));
                // echo $newhash;
                if ($newhash == $hash) {
                    $GLOBALS['db']->Execute("update " . $GLOBALS['Tables']['user'] . " set status=1,allowlogin=1 where userid=" . (int) $row['userid']);
                    return true;
                }
            }
            return false;
    ...
    {
        $rs = $GLOBALS['db']->Execute("Select * from " . $GLOBALS['Tables']['user'] . " where username='" . $username . "' and email='" . $email . "'");
        if (\TAS\DB::Count($rs) > 0) {
            while ($row = $GLOBALS['db']->Fetch($rs)) {
                $row['password'] = substr(md5(uniqid()), 0, 8);
                $GLOBALS['db']->Execute("Update " . $GLOBALS['Tables']['user'] . " set password='" . $row['password'] . "' where userid=" . (int) $row['userid']);
    ...
    public static function Permission_AuthUser($username, $password, $location, $corelocation)
    {
        if (trim($username) == "" || trim($password) == "") {
            ...
            if ($userid) {
                    $GLOBALS['db']->Execute("UPDATE " . $GLOBALS['Tables']['user'] . " set lastlogin=now() where memberid=" . $userid);
                    $GLOBALS['user'] = new \TAS\User($userid);
